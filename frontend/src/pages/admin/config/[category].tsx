...
export default function AppShellDemo() {
  ...
  const isEditingAllowed = (): boolean => {
    // If list is loading or empty, assume editing is allowed or irrelevant
    if (!configVariables || configVariables.length === 0) return true;
    return configVariables[0].allowEdit;
  };
  // -----------------------------
          ...
          {!configVariables ? (
            ...
            <>
              <Stack>
                ...
                {/* Check if variables exist before mapping, otherwise show "No settings" */}
                {configVariables.length === 0 && (
                   <Text color="dimmed">No settings available for this category.</Text>
                )}
