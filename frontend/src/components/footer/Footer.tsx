  ...
  return (
    <MFooter height="auto" py={6} px="xl" zIndex={100}>
      <SimpleGrid cols={isMobile ? 2 : 3} m={0}>
        {/* Spacer for desktop layout to keep text centered */}
        {!isMobile && <div></div>}
        
        {/* --- CHANGED: Removed Pingvin Share Link --- */}
        <Text size="xs" color="dimmed" align={isMobile ? "left" : "center"}>
          © 2026 WeTransfer University Project
        </Text>

        {/* Legal Links (Only shows if enabled in config, usually hidden) */}
        <div>
