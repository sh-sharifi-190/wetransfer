...
export class OAuthService {
  ...
  availableProviders(): Record<string, OAuthProvider<unknown>> {
    return Object.fromEntries(
      Object.entries(this.oAuthProviders)
        ...
        .filter(([providerName]) => this.config.get(`oauth.${providerName}-enabled`))
    );
  }
  ...
  async signIn(user: OAuthSignInDto, ip: string) {
    ...
    if (oauthUser) {
      ...
      // ✅ فقط یک آرگومان به generateToken می‌فرستیم
      return this.auth.generateToken(updatedUser);
    }
  ...
  private async signUp(user: OAuthSignInDto, ip: string) {
    ...
    if (existingUser) {
      ...
      // ✅ فقط یک آرگومان به generateToken می‌فرستیم
      return this.auth.generateToken(existingUser);
    }
